...
app.get('/api/inventory/search', authenticateToken, async (req, res) => {
    try {
        ...
        console.log(`🔍 Searching for "${searchTerm}" in inventory items`);
        
        const query = `
            SELECT id, sku, name, type, stock_level, unit_type
            FROM InventoryItems
        ...
        console.log(`🔍 Found ${result.rows.length} results for search "${searchTerm}"`);
...
app.post('/api/forecast', authenticateToken, checkPermission('view_reports'), async (req, res) => {
    try {
      ...
      const salesQuery = `
        ...
        GROUP BY it.sku, it.name, it.current_stock, it.type
        ORDER BY it.name;
      `;
      ...
      const mmr = mmrResult.rows.reduce((acc, r) => {
        ...
        acc[r.product_sku][r.ingredient_sku] = { 
          quantity: parseFloat(r.quantity) || 0,
          name: r.ingredient_sku 
        };
        return acc;
      }, {});
      ...
      const GROWTH_FACTOR = 1.1; // 10% growth
      ...
      for (const item of sales) {
        ...
        const sold30d = Number(units_sold_30d) || 0;
        const sold30_60d = Number(units_sold_30_60d) || 0;
        const sold60_90d = Number(units_sold_60_90d) || 0;
        const sold90d = Number(units_sold_90d) || 0;
  
        // Calculate daily averages
        const avgDaily30d = sold30d / DAYS_30;
        const avgDaily60d = (sold30d + sold30_60d) / DAYS_60;
        const avgDaily90d = sold90d / DAYS_90;
  
        // Calculate forecasts with growth factor
        const forecast30d = Math.ceil(avgDaily30d * FORECAST_DAYS * GROWTH_FACTOR);
        const forecast60d = Math.ceil(avgDaily60d * FORECAST_DAYS * GROWTH_FACTOR);
        const forecast90d = Math.ceil(avgDaily90d * FORECAST_DAYS * GROWTH_FACTOR);
  
        const monthlyForecast = forecast30d;
        const batchUnits = Math.ceil(monthlyForecast / BATCH_SIZE) * BATCH_SIZE;
        ...
        // Production planning
        if (stock < monthlyForecast) {
          const productionUnits = Math.ceil((monthlyForecast - stock) / BATCH_SIZE) * BATCH_SIZE;
          production.push({
            product_id,
            product_name,
        ...
        for (const [material, { quantity: qtyPerUnit, name }] of Object.entries(mmrItems)) {
          try {
            ...
            const currentMatStock = matStockResult.rows[0]?.stock_level ? Number(matStockResult.rows[0].stock_level) : 0;
            const needed = batchUnits * qtyPerUnit;
  
            if (currentMatStock < needed) {
              reorders.push({
                material,
                material_name: name,
                quantity: Math.ceil(needed - currentMatStock),
                order_by: new Date(Date.now() - (DEFAULT_LEAD_TIME - FORECAST_DAYS) * 86400000)
                  .toISOString().split('T')[0]
              });
            }
          } catch (err) {
            console.error(`Error checking stock for material ${material}: ${err.message}`);
          }
        }
      }
      ...
      try {
        ...
        const totalFinishedGoods = parseInt(countResult.rows[0].total_products) || 0;
  
        console.log(`Forecast diagnostic: Found ${totalFinishedGoods} total finished goods`);
        console.log(`Returning ${Object.keys(forecast).length} products out of ${totalFinishedGoods}`);
        ...
        console.log("Sample finished goods:", 
          sampleResult.rows.map(row => `${row.sku}: ${row.name} (${row.stock_level})`).join(", ")
        );
      } catch (diagError) {
